...
1. **许可来源干净。** 每一行代码要么是自己写的，要么来自有明确许可的上游。这是这个项目存在的首要理由：现有的 `skysb` 有 96% 的代码继承自没有 LICENSE 的 `rwnode-gosingbox`，没有分发权。
...
沿用现有的 `sing-box-fork`（8 文件补丁，已有 `MODIFICATIONS.md`），它本来就是干净的 GPL-3.0 衍生。
...
2. **用户名快照必须原子发布并做边界检查。** VLESS 和 SS 用**下标**在连接路径上查用户名，下标由 service 发放、与用户切片分别更新，删用户时长度对不上会索引越界打崩节点。**对策：** `atomic.Pointer` 快照 + 越界返回空串。（这条已在 `sing-box-fork` 里修好，重写节点时照抄自己的补丁即可。）
...
M4 是真正的验证点 —— 到那一步为止，现有的 `skysb` 部署继续跑，不会有停机。

## 12. 待定

1. **项目名。** `skysb` 是现有的；新项目沿用还是另起？两个仓库的名字也一并定（如 `xxx-panel` / `xxx-node`）。
