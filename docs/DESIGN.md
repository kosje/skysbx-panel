...
`node-token` 在 panel 建节点时生成，**一次性显示，只存 bcrypt 哈希**。没有证书交换，
没有密钥派生。token 可以随时在节点页「换 token」。
...
`settings` 里目前放三样东西：会话签名密钥、管理员凭据、路由策略（§9）。
...
四种状态：**已生效**（节点确认在跑）/ **确认中…**（还在等）/ **未生效**（节点报了错，
错误原文直接显示在页面顶部）/ **节点离线**（没有报告，沉默不等于失败）。
